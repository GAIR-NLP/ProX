...
from transformers import (AutoConfig, AutoModelForCausalLM, AutoTokenizer,
                          GPTNeoXForCausalLM, OPTForCausalLM, StoppingCriteria)

from eval.dispatch_openai_requests import (dispatch_openai_chat_requesets,
...
def load_hf_lm_and_tokenizer(
    ...
    # TODO Hardcode for OLMO @koalazf99
    config = None
    if "OLMo" in model_name_or_path:
        try:
            config = None
            config = AutoConfig.from_pretrained(model_name_or_path, tie_word_embeddings=False, weight_tying=False)
        except Exception as e:
            raise Exception(e)
    if gptq_model:
        ...
        if device_map:
            ...
            a = [name for name, _ in model.state_dict().items()]
            print(a)
        else:
            model = AutoModelForCausalLM.from_pretrained(
                model_name_or_path,
