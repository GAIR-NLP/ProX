...
class DropoutAddLayerNormFn(torch.autograd.Function):
    ...
    def forward(
        ...
        if not return_dmask:
            ...
            dmask = (
                dmask.view(x0.shape) if dropout_p > 0.0 else torch.ones(x0.shape, dtype=torch.uint8, device=x0.device)
            )
...
class DropoutAddLayerNormParallelResidualFn(torch.autograd.Function):
    ...
    def forward(
        ...
        if not return_dmask:
            ...
            dmask0 = (
                dmask0.view(x0.shape) if dropout_p > 0.0 else torch.ones(x0.shape, dtype=torch.uint8, device=x0.device)
            )
            dmask1 = (
...
class RMSNorm(torch.nn.Module):
    ...
    def __init__(self, size: int, dim: int = -1, eps: float = 1e-5, add_unit_offset: bool = False) -> None:
        super().__init__()
        self.weight = torch.nn.Parameter(torch.ones(size))
        self.eps = eps
        self.dim = dim
