...
parser.add_argument(
    "MODEL",
    type=str,
    default="/cpfs01/user/liupengfei/fzhou/TinyLlama/hf_models/tiny_LLaMA_0_7b_20B",
    nargs="?",
...
parser.add_argument(
    ...
    type=str,
    default="/cpfs01/user/liupengfei/fzhou/TinyLlama/hf_models/tiny_LLaMA_0_7b_20B",
    help="Path to the output folder (default: models/{model_name}_safetensors).",
)
parser.add_argument(
    "--max-shard-size",
    type=str,
    default="3GB",
    help="Maximum size of a shard in GB or MB (default: %(default)s).",
)
parser.add_argument(
    "--bf16",
    action="store_true",
...
if __name__ == "__main__":
    ...
    print(
        f"Saving the converted model to {out_folder} with a maximum shard size of {args.max_shard_size}..."
    )
    model.save_pretrained(
        out_folder, max_shard_size=args.max_shard_size, safe_serialization=True
    )
